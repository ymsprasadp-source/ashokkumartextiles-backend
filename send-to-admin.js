// Send Order Confirmation Email to aktexmsp9@gmail.com
...
async function sendOrderConfirmation() {
  ...
  try {
    // Fetch the most recent order with user details
    ...
    const userEmail = lastOrder.users?.email || 'unknown@example.com';
      ...
      <body>
        <div class="container">
          ...
          <div class="footer">
            ...
            <p style="margin: 5px 0 0 0; font-size: 12px;">Thank you for shopping with us!</p>
          </div>
        </div>
      </body>
      </html>
    ...
    const mailOptions = {
      ...
      to: 'aktexmsp9@gmail.com',
      subject: `✅ Order Confirmation #${lastOrder.id} - Ashok Kumar Textiles (₹${lastOrder.amount})`,
      html: orderEmailHTML,
      text: `Order Confirmation - Ashok Kumar Textiles\n\nOrder ID: ${lastOrder.id}\nAmount: ₹${lastOrder.amount}\nStatus: ${lastOrder.status || 'Processing'}\nItems: ${orderItems.length}\n\nThank you for your order!\n\nCustomer: ${userEmail}\nDate: ${new Date(lastOrder.created_at).toLocaleDateString('en-IN')}`
    };

    console.log('📤 Sending order confirmation email...');
    console.log('📧 Recipient: aktexmsp9@gmail.com');
    
    const info = await transporter.sendMail(mailOptions);

    console.log('✅ Order confirmation email sent successfully!');
    console.log('📬 Message ID:', info.messageId);
    console.log('🎉 Check aktexmsp9@gmail.com inbox');
    console.log('\n📋 Order Summary:');
...
sendOrderConfirmation()
  .then(() => {
    console.log('\n✨ Email sent successfully to aktexmsp9@gmail.com!');
    process.exit(0);
  })
  .catch((error) => {
    console.error('\n💥 Failed:', error);
